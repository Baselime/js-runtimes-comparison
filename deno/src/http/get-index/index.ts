function buildResponse(data: Record<string, any>, code: number) {
  return {
    statusCode: code,
    headers: {
...
export async function handler(event: Record<string, any>, context: Record<string, any>) {
  ...
  try {
    ...
    if (rand < threshold) {
      console.error(JSON.stringify({ message: "Backend error", extra: { rand, requestId, name } }));
    ...
    if (!pokemon) {
      ...
      console.log(JSON.stringify({ message: "RESPONSE", extra: { data, code: 404, requestId, name } }));
      return buildResponse(data, 404);
    }

    const data = { pokemon };
    console.log(JSON.stringify({ message: "RESPONSE", extra: { data, code: 200, requestId, name } }));
    ...
    console.error(JSON.stringify({ message: "RESPONSE", extra: { name, code: 500, requestId } }));
